...
function getPowerFunction(exponent) {
  return function powerValue(value) {
  ...
  return function log(...args) {
    ...
    args.forEach((i) => {
      if (Array.isArray(i)) {
        resLog += '[';
        i.forEach((j) => {
          resLog += `${JSON.stringify(j)},`;
        });
        resLog = resLog.slice(0, -1);
        resLog += '],';
      } else {
        resLog += i;
      }
    });
    logFunc(`${func.name}(${resLog}) starts`);
    const res = func(...args);
    logFunc(`${func.name}(${resLog}) ends`);
    return res;
  };
}
/**
